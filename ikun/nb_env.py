...
def show_bytes_figure(image_base64: str, interaction_type: Literal["ipython", None]):
...
def add_output_to_cell(cell: NotebookNode, output: str):
...
def display_markdown(content: str):
    ...
    if remaining_text:
        content_panels.append(
            Panel(Markdown(remaining_text), style=style, box=MINIMAL))
...
def parse_outputs(outputs: list[str], keep_len: int = 2000) -> Tuple[bool, str]:
    ...
    for i, output in enumerate(outputs):
        ...
        if output["output_type"] == "stream" and not any(
            ...
            if "image/png" in output["data"]:
                show_bytes_figure(
                    output["data"]["image/png"], "ipython" if is_ipython() else "terminal")
            else:
                logger.info(
